...
Kerberos permet aux utilisateurs d'utiliser un service sur le réseau sans envoyer son mot de passe, et sans que le serveur ait besoin de connaitre les mots de passe de tout le monde. C'est une authentification centralisée.
...
We are in Active Directory context, so the KDC is also the Domain Controller (DC). The KDC contains all the domain nformation, including the secrets of each service, machine, user. Thus, except for the DC, everyone only know his own secret, and therefore do not know the secrets of the other objects in Active Directory.

[! [DC with keys](/assets/uploads/2018/05/keys.png)
...
In order to perform this TGT request, `pixis` will send its name to the KDC as well as the exact time of the request, encrypted with a hashed version of his password.
...
- The **TGT**, containing various information like:
  ...
  TGT will be encrypted with the KDC key. Thus, **only the KDC is able to decipher and read the content of this ticket**.
...
The client will receive this message, and will be able to decrypt the first layer to get the session key created for communication with the service, as well as the ticket generated to use of this service. This ticket is commonly called **Ticket-Granting-Service** (TGS).
